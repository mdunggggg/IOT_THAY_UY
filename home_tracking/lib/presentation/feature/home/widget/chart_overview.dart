...
class _ChartOverviewState extends State<ChartOverview> {
  ...
  SfCartesianChart _buildMultipleAxisLineChart() {
    return SfCartesianChart(
      ...
      primaryXAxis:  const NumericAxis(
          majorGridLines: MajorGridLines(width: 0), maximumLabels: 10,),
  ...
  List<CartesianSeries<DataSensorModel, int>>
      _getMultipleAxisLineSeries() {
    return <CartesianSeries<DataSensorModel, int>>[
      LineSeries<DataSensorModel, int>(
          dataSource: widget.list,
          xValueMapper: (DataSensorModel sales, _) => sales.id ?? 0,
  ...
  String convertToTime(int? milliseconds) {
    return DateFormat('HH:mm').format(DateTime.fromMillisecondsSinceEpoch(milliseconds ?? 0));
  }
